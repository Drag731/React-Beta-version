...
var likeImg = {
    like: 'http://ic.pics.livejournal.com/ash9577/48888313/5949/5949_original.jpg',
    dislike: 'https://ic.pics.livejournal.com/ash9577/48888313/6334/6334_original.jpg'
}
...
class Description extends React.Component {
    ...
    render() {
        const arrayOfActors = this.props.activeMovie.actors;
        const arrayOfGenres = this.props.activeMovie.genres;
        const actors = (
            <p> Actors: {arrayOfActors.map((user) =>
                    <b> {user} </b>
                    )}
            </p>
        );
        const genres = (
            <p> Genres: {arrayOfGenres.map((user) =>
                    <b> {user}, </b>
                    )}
            </p>
        );
        ...
        return (
            <div>
                <div className="description-title">
                    ...
                    <Stars movies={this.props.movies}/>
                </div>
                <div className="description-about">
                    <div>
                        <img className="description-img" src={this.props.activeMovie.posterUrl} />
                    </div>
...
class ItemMovie extends React.Component {
    constructor(props){
        super(props);
        this.state = {lilkeCount: this.props.movies.likes};
    ...
    onClick() {
        var title = this.props.movies.id;
        this.props.change(title);
    }

    likeUp() {
        this.setState({lilkeCount: this.state.lilkeCount += 1});
    }
    ...
    render() {
        return (
            <div key={this.props.movies.id} className="movie-item">
                <div className="likes">
                    <img onClick={this.likeUp} className="likes-img" src={likeImg.like} />
                    <img onClick={this.likeDown} className="likes-img" src={likeImg.dislike} />
                    likes
                </div>
                
                <div className="poster">
                    <h3 onClick={this.onClick} className="movie-name">{this.props.movies.title}</h3>
                    <img className="poster-img" src={this.props.movies.posterUrl} />
                </div>
                <div className="count-likes">{this.state.lilkeCount}</div>
                <div className="stars">
                    <Stars movies= {this.props.movies}/>
                </div>
            </div>
...
class App extends React.Component {
    ...
    render() {
        ...
        const activeMovie = movies.filter((el) => el.id === parseInt(this.state.activeId))[0];
        return (
            <div>
                <div className="movies">
                    ...
                    <div className="movies-item">
                        {movies.map((movie) => {
                            return (<ItemMovie movies={movie} change={this.change}/> );
                        })}
                    </div>
                </div>
                <div className="description">
                    <Description activeMovie={activeMovie} change={this.change} movies={movies}/>
                </div>
            </div>
        );
    }
